...
export function BookingListItem({ booking, currentView, onClick }: BookingListItemProps) {
  return (
    <div
      onClick={onClick}
      className="bg-white rounded-lg border border-slate-200 hover:border-indigo-300 hover:shadow-sm cursor-pointer transition h-[152px] overflow-hidden"
    >
            ...
            <div className="w-[300px] shrink-0 leading-snug pr-4 mr-5 text-[11px]">
              ...
              <div className="flex items-center gap-0.5 mt-0.5 uppercase overflow-hidden">
                {booking.stage === 'cancelled' ? (
                  <span className="px-1.5 py-px rounded text-[9px] font-semibold shrink-0" style={{ backgroundColor: STAGE_CONFIG.cancelled.bg, color: STAGE_CONFIG.cancelled.color }}>Cancelled</span>
                ) : booking.stage === 'transferred' ? (<>
                  <span className="px-1.5 py-px rounded text-[9px] font-semibold shrink-0" style={{ backgroundColor: STAGE_CONFIG.transferred.bg, color: STAGE_CONFIG.transferred.color }}>Transferred</span>
                  {booking.refund_status && booking.refund_status !== 'ไม่มี' && (
                    <span className="px-1.5 py-px rounded text-[9px] font-semibold shrink-0 bg-amber-50 text-amber-600">Refund</span>
                  )}
                  {(!booking.water_meter_change_date || !booking.electricity_meter_change_date) && (
                    <span className="px-1.5 py-px rounded text-[9px] font-semibold shrink-0 bg-blue-50 text-blue-600">Meter</span>
                  )}
                  {!booking.handover_document_received_date && (
                    <span className="px-1.5 py-px rounded text-[9px] font-semibold shrink-0 bg-emerald-50 text-emerald-600">Freebie</span>
                  )}
                  {!booking.handover_document_received_date && (
                    <span className="px-1.5 py-px rounded text-[9px] font-semibold shrink-0 bg-red-50 text-red-500">Con-Pending</span>
                  )}
                </>) : (() => {
                  ...
                  return (<>
                    {!creditDone && (
                      <span className="px-1.5 py-px rounded text-[9px] font-semibold shrink-0" style={{ backgroundColor: STAGE_CONFIG.credit.bg, color: STAGE_CONFIG.credit.color }}>Credit</span>
                    )}
                    {!inspDone && (
                      <span className="px-1.5 py-px rounded text-[9px] font-semibold shrink-0" style={{ backgroundColor: STAGE_CONFIG.inspection.bg, color: STAGE_CONFIG.inspection.color }}>Inspection</span>
                    )}
                    {creditDone && inspDone && (
                      <span className="px-1.5 py-px rounded text-[9px] font-semibold shrink-0" style={{ backgroundColor: STAGE_CONFIG.ready.bg, color: STAGE_CONFIG.ready.color }}>Ready</span>
                    )}
                  </>);
                })()}
              </div>
              ...
              <div className="flex items-center gap-1 mt-auto pt-0.5 border-t border-slate-100">
                <div className="flex flex-wrap gap-0.5 flex-1 min-w-0">
                  {booking.banks_submitted.map(bs => (
                    <span key={bs.bank} className={`inline-flex px-1 py-px text-[10px] font-bold text-white uppercase ${BCLR[bs.bank] || 'bg-slate-500'}`} title={[bs.bank, bs.result, bs.approved_amount ? `฿${formatMoney(bs.approved_amount)}` : ''].filter(Boolean).join(' · ')}>
                      {bs.bank}
                    </span>
                  ))}
                </div>
